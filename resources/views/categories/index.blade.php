                            ...
                            <tr>
                                ...
                                <td class="px-4 py-3 text-right">
                                    @if (Auth::user()->can('update', $c) || Auth::user()->can('delete', $c))
                                        <x-actions-menu>
                                            @can('update', $c)
                                                <a href="{{ route('categories.edit', $c) }}" class="menu-item">Edit</a>
                                            @endcan
                                            @can('delete', $c)
                                                <form action="{{ route('categories.destroy', $c) }}" method="POST" data-confirm="Delete this category? This cannot be undone.">
                                                    @csrf @method('DELETE')
