        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('companies.update', $company) }}"
                ...
                <div>
                    ...
                    <x-text-input id="name" name="name" :value="old('name', $company->name)" required class="mt-1 block w-full" />
                    <x-input-error :messages="$errors->get('name')" class="mt-1" />
                </div>
                <div>
                    <x-input-label for="code" value="Code *" />
                    <x-text-input id="code" name="code" :value="old('code', $company->code)" required class="mt-1 block w-full" />
                    <x-input-error :messages="$errors->get('code')" class="mt-1" />
                </div>
                <div>
                    <x-input-label for="address" value="Address" />
                    <x-text-input id="address" name="address" :value="old('address', $company->address)" class="mt-1 block w-full" />
